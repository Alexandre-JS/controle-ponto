...
export class SyncService {
  ...
  // Flag para modo manual: se true, não sincroniza automaticamente ao reconectar
  private manualSyncOnly = true;
  ...
  constructor(
    ...
    // Removido startAutoSync para garantir modo 100% manual
  ...
  private setupNetworkListeners(): void {
    this.networkService.isOnline$.pipe(
      ...
      if (isOnline) {
        if (this.manualSyncOnly) {
          console.log('Conexão restaurada, modo manual: não sincronizando automaticamente.');
        } else {
  ...
  private async processSyncItem(item: SyncQueueItem): Promise<void> {
    ...
    // Função para validar se o id é UUID v4
    ...
    try {
      ...
      if (tableError) {
        ...
        // ... (restante do tratamento de erro da tabela permanece igual)
        // (copiar bloco existente)
        // ...
        throw new Error(`Tabela ${tableName} não existe ou não é acessível: ${tableError.message} (Código: ${tableError.code})`);
      }

      // Lógica especial para update de attendance/work_schedule com id não-UUID
      if (item.action === 'update' && ['attendance', 'work_schedule'].includes(item.type)) {
        if (!cleanData.id || !isValidUUID(cleanData.id)) {
          // Se o id não é UUID, remover o campo id antes do insert
          console.log(`ID local (${cleanData.id}) não é UUID. Fazendo insert em vez de update para ${item.type}.`);
          ...
          const { data, error: insertError } = await supabase
            ...
            .select();
          if (insertError) {
            console.error(`Erro ao inserir ${item.type} (fallback de update):`, insertError);
            throw insertError;
          }
          if (data && data[0] && data[0].id) {
            // Atualizar id localmente (attendance/work_schedule)
            const newId = data[0].id;
            if (item.type === 'attendance') {
              this.localStorageService.saveAttendance({ ...cleanData, id: newId, synced: true });
            } else if (item.type === 'work_schedule') {
              this.localStorageService.saveWorkSchedule({ ...cleanData, id: newId, synced: true });
            }
            this.localStorageService.removeSyncQueueItem(item.id);
            console.log(`ID local atualizado para UUID: ${newId}`);
          }
          return;
        }
      }
  ...
  private startAutoSync(): void {
    // Só ativa auto sync se manualSyncOnly for false
    if (this.manualSyncOnly) {
      console.log('Modo manual: sincronização automática por timer desativada.');
      return;
    }
